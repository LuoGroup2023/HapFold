...
void Print_H(hifiasm_opt_t* asm_opt)
{
    ...
    fprintf(stderr, "    --no-write-cache\n");
    fprintf(stderr, "                 do not write corrected-read and overlap restart caches\n");
...
int CommandLine_process(int argc, char *argv[], hifiasm_opt_t* asm_opt)
{
    ...
    while ((c = ketopt(&opt, argc, argv, 1, "hvt:o:k:w:m:n:r:a:b:z:x:y:p:c:d:M:P:if:D:FN:1:2:3:4:5:l:s:O:eu:", long_options)) >= 0) {
        ...
        **/
            ...
            if (asm_opt->hybrid_min_unique_anchors < 1)
                asm_opt->hybrid_min_unique_anchors = 1;
        } else if (c == 369) {
            asm_opt->hybrid_unique_weight = atof(opt.arg);
            if (asm_opt->hybrid_unique_weight < 0.0)
                asm_opt->hybrid_unique_weight = 0.0;
        } else if (c == 370) {
            asm_opt->hybrid_unique_bonus_cap = atof(opt.arg);
            if (asm_opt->hybrid_unique_bonus_cap < 0.0)
                asm_opt->hybrid_unique_bonus_cap = 0.0;
            if (asm_opt->hybrid_unique_bonus_cap > 1.0)
                asm_opt->hybrid_unique_bonus_cap = 1.0;
